...
import { getDiscoverPosts, getPosts, getUserPosts, onEdit } from '@/redux/features/post';
...
export default function CardHeader({ post, inDiscover, inProfile, inHome }: CardHeaderProps) {
    ...
    const deleteHandler = async () => {
        try {
            await axios.delete(`${process.env.API}/api/post/${post._id}`, { headers: { Authorization: auth.access_token } });
            inDiscover && dispatch(getDiscoverPosts({ access_token: auth.access_token as string, page: 1 }));
            inHome && dispatch(getPosts({ access_token: auth.access_token as string, page: 1 }));
            inProfile && dispatch(getUserPosts({ access_token: auth.access_token as string, page: 1, id: auth.user?._id as string }));
        } catch (error) {
                    ...
                    >
                        {
                            ...
                            (
                                <>
                                    ...
                                    <DropdownMenu.Item onClick={ deleteHandler } className="my-1 group text-[13px] leading-none text-violet11 rounded-[3px] flex items-center px-[5px] relative select-none outline-none data-[disabled]:text-mauve8 data-[disabled]:pointer-events-none data-[highlighted]:bg-violet9 data-[highlighted]:text-violet1 hover:bg-neutral-600 hover:text-white">
                                        <div className="flex gap-x-2 items-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={ 1.5 } stroke="currentColor" className="w-6 h-6">
