...
export default function StatusModal({
  ...
  const updateHandler = async (e: IFormProps) => {
    ...
    try {
      ...
      await axios.put(
        `${process.env.API}/api/post/${post.postToEdit?._id}`,
        { content, images: [...imagesNotToUpload, ...(media as imgType[])] },
        { headers: { Authorization: auth.access_token } }
      );
      auth.access_token &&
        isInHomePage &&
        !isInProfilePage &&
        dispatch(getPosts({ access_token: auth.access_token, page: 1 }));
      console.log({
        id: post.postToEdit?._id as string,
  ...
  return (
    <div className="fixed top-0 left-0 bg-black/50 w-full h-screen overflow-auto z-[9]">
      <form
        onSubmit={
          post.onEdit && post.postToEdit ? updateHandler : submitHandler
        }
      ...
      >
        <div className="flex justify-between items-center border-b border-gray-200 mb-2 py-3">
          ...
          <span
            className="cursor-pointer text-3xl font-black -translate-y-1"
            onClick={() => dispatch(setStatusModalShow(false))}
          >
            &times;
          </span>
        </div>
        ...
        <div className="">
          ...
          {stream && (
            <div className="relative">
              ...
              <div
                ...
                className="cursor-pointer absolute top-1 right-1 bg-main/30 rounded-full p-2 text-4xl w-10 h-10 font-bold flex justify-center items-center"
              >
          ...
          <div
            className={`max-h-64 w-full overflow-y-auto grid  gap-3 place-items-center grid-cols-3`}
          >
            {images.map((img, index) => (
              <div className="w-full h-full relative" key={index}>
                <img
                  ...
                  alt="images"
                  className="w-full h-full block object-contain border-gray-200 border p-1 rounded-md"
                />
                <span
                  onClick={() => deleteImage(index)}
                  className="cursor-pointer text-xl font-bold absolute top-1 right-1 bg-white border border-red-500 text-red-500 rounded-full px-2 py-0.5 transition-colors duration-150 hover:bg-red-500 hover:text-white hover:border-white pt-0 ease-out"
                >
                  &times;
                </span>
              </div>
        ...
        <button className="btn-dark text-center">
          {post.onEdit && post.postToEdit ? "Update" : "Post"}
        </button>
      </form>
    </div>
  );
