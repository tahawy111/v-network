...
export default function Search({ }: SearchProps) {
    ...
    return <form onSubmit={(e) => e.preventDefault()} className={ `relative w-full mx-1 md:w-1/3 text-black dark:text-white` }>
        ...
        <span onClick={() => {setUsers([]); setSearch("")}} className={`absolute text-red-500 text-3xl left-full -translate-x-6 top-full -translate-y-full leading-10 cursor-pointer ${search || users && users?.length > 0 ? "block" : "hidden"}`}>&times;</span>
